...
**Build status:** All 14 planned assessment modules are implemented: Identity, ConditionalAccess, PrivilegedAccess, Devices, IdentityGovernance, Applications, HybridIdentity, Monitoring, Defender, ThreatProtection, SecurityCompliance, DataProtection, Collaboration, CloudAppSecurity. ThreatProtection, SecurityCompliance, DataProtection, and Collaboration are built on the read-only Exchange Online / Security & Compliance (IPPS) connection surface. CloudAppSecurity is Graph-only and is explicitly a best-effort Microsoft Secure Score proxy — Microsoft Graph exposes no Defender for Cloud Apps configuration API, so this domain does not assess MCAS policies, app risk scores, or OAuth app governance; a dedicated portal review is recommended alongside it. The module carries a 650+ test Pester/QA suite with an 85%+ coverage gate, but that coverage is built on mocked Graph/Exchange responses — it does not substitute for live-tenant validation. The first real engagement run against a production tenant (August 2026) surfaced and required fixing several issues in rapid succession: a malformed API response from one module could abort the *entire* multi-module run rather than degrading gracefully (fixed in 0.3.3, which also hardened every module's assessor against this class of failure); the device-code sign-in prompt could silently fail to display, leaving the connection to time out with no visible cause (fixed across 0.3.1–0.3.2); and the CI/CD release pipeline itself had never successfully auto-published a version before that day. Treat each release as reasonably well-tested in isolation, not as proven against every tenant configuration and Graph/Exchange SDK version combination — report anything unexpected. Remaining roadmap: richer report packaging (PDF, Excel, dashboard outputs) if required.
...
For `ThreatProtection`, `SecurityCompliance`, `DataProtection`, or `Collaboration` **when using CBA (app-only)**, the app's service principal also needs Exchange Online / Security & Compliance role groups. `Get-ZTAssessExchangeOnlineRoleGuidance` lists them; `Grant-ZTAssessExchangeOnlineRole` can grant them (run by an account with sufficient Exchange Online rights) as an alternative to the tenant's own Exchange administrator granting them manually — see [step 3a](docs/Authentication.md#3a-grant-exchange-online--purview-role-groups-only-for-securitycompliance-collaboration-dataprotection-threatprotection). **This step doesn't apply to `-UseDeviceCode` engagements** — device-code mode never connects as an app, so it's the signed-in consultant's own account that needs the equivalent permissions instead; see the linked section for details.
