...
- Public module names stay user-facing. Internal check/settings/scoring domains map `Applications` to `ApplicationSecurity` and `Monitoring` to `MonitoringDetection`; use the internal domains in `source/Checks` and `source/Settings`. `Defender` uses the same name for both; its device-onboarding check (DF-002) is a best-effort Secure Score proxy (Graph exposes no direct onboarded-machine count) and degrades to `NotAssessed` if Microsoft renames the underlying control.
