...
- Public module names stay user-facing. Internal check/settings/scoring domains map `Applications` to `ApplicationSecurity` and `Monitoring` to `MonitoringDetection`; use the internal domains in `source/Checks` and `source/Settings`. `Defender` and `CloudAppSecurity` use the same name for both; Defender's device-onboarding check (DF-002) and CloudAppSecurity's setup check (CAS-001) are best-effort Secure Score proxies (Graph exposes no direct onboarded-machine count or Defender for Cloud Apps configuration API) and degrade to `NotAssessed` if Microsoft renames the underlying control. `CloudAppSecurity` shares its collected data with `Defender` — there is no separate `CloudAppSecurity` collector.
...
```powershell
# Optional pre-flight: install RequiredModules.psd1 build modules if missing (idempotent).
# Run this first when dependency restore cannot fetch InvokeBuild, PSScriptAnalyzer,
...
- A second, lazy, read-only connection surface exists for Exchange Online / Security & Compliance (IPPS), used only by modules whose data is not in Microsoft Graph (`ThreatProtection`, `SecurityCompliance`, `DataProtection`, `Collaboration`) — all four are implemented (4 checks each). `Collaboration`'s v1 scope excludes SharePoint tenant sharing settings (`Get-SPOTenant`), which would need a third connection surface. In app-only mode it reuses the Graph app-only certificate; in device-code mode (`-UseDeviceCode`) it obtains a separate Exchange Online access token via a manual OAuth device-code flow (`Get-ZTAssessExchangeOnlineDeviceCodeToken`) — since the Security & Compliance session-establishing cmdlet has no device-code switch of its own — using a configurable well-known Microsoft first-party client ID (`Settings/settings.psd1` → `ExchangeOnline.DeviceCodeClientId`). It is skipped only for plain interactive delegated sign-in, and never fails the overall connection; all calls go through the allow-listed `Invoke-ZTAssessExoRequestWrapper` (`Get-*` cmdlets only, enforced by `tests/QA/ReadOnly.tests.ps1`). `-Organization`/`ZTASSESS_ORGANIZATION` identify the Exchange Online tenant domain. The read-only assessment module never grants the Exchange Online role groups these modules need — `Get-ZTAssessExchangeOnlineRoleGuidance` documents them. `Grant-ZTAssessExchangeOnlineRole` (in `EntraZTAssess.Provisioning`, a Graph/EXO write) can optionally grant them instead of a manual admin step. See `docs/Authentication.md`.
...
- The **only** place Graph and Exchange Online write operations are permitted is the `EntraZTAssess.Provisioning` module (source under `scripts/` in this repository; functions `New-ZTAssessCertificate`, `New-ZTAssessAppRegistration`, `Grant-ZTAssessExchangeOnlineRole`), which is admin-run and one-time. It is never shipped as part of `Get-EntraZTAssess`'s `Install-Module` — it is published as its own, separately versioned PSGallery package (`Install-Module EntraZTAssess.Provisioning`) so the read-only assessment module never carries write-capable code, with cloning `scripts/EntraZTAssess.Provisioning` still supported for anyone auditing source first. `New-ZTAssessAppRegistration`'s elevated setup permissions are requested least-privilege: `Application.ReadWrite.All` is always requested, and `AppRoleAssignment.ReadWrite.All` is additionally requested only when `-GrantAdminConsent` is supplied. These are setup-only and are not assessment scopes. It also refuses to silently duplicate an existing application registration with the same `-DisplayName` unless `-Force` is supplied. `Grant-ZTAssessExchangeOnlineRole` performs Exchange Online RBAC role-group writes (`New-ServicePrincipal`, `Add-RoleGroupMember`) and must be run by an account with sufficient Exchange Online / Security & Compliance administrative rights; it is idempotent, skipping a service principal or role-group membership that already exists. The module under `source/` remains read-only and holds only the read-only Application permissions the assessment app is granted; `tests/QA/ReadOnly.tests.ps1` additionally forbids Graph write SDK cmdlets (`New-Mg*`/`Set-Mg*`/`Remove-Mg*`/`Update-Mg*`) under `source/`. The read-only `Get-ZTAssessProvisioningStep` cmdlet surfaces the provisioning steps as guidance.
